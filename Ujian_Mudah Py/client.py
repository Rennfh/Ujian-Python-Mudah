...
def log_client_activity_safe(thread_name, message):
    ...
    with client_log_lock:
        ...
        log_msg = f"[{timestamp}] [{thread_name}] {message}\n"
...
def request_stock_from_api(sku, current_thread_name):
    ...
    def request_stock_from_api(sku, current_thread_name):
        ...
        log_client_activity_safe(current_thread_name, f"Mengirim permintaan ke API untuk SKU: {sku}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                log_client_activity_safe(
                    current_thread_name,
                    f"Berhasil! Stok '{data.get('name', 'N/A')}' adalah {data.get('stock', 0)} unit di {data.get('warehouse', 'N/A')}"
                )
            elif response.status_code == 404:
                data = response.json()
                log_client_activity_safe(
                    current_thread_name,
                    f"Error: SKU {sku} tidak ditemukan. Pesan: {data.get('message', 'Not found')}"
                )
            else:
                log_client_activity_safe(
                    current_thread_name,
            ...
            log_client_activity_safe(current_thread_name, f"Timeout saat menghubungi API untuk SKU {sku}")
        except requests.RequestException as e:
            log_client_activity_safe(current_thread_name, f"Permintaan gagal untuk SKU {sku}: {e}")
        except Exception as e:
            log_client_activity_safe(current_thread_name, f"Kesalahan tak terduga untuk SKU {sku}: {e}")
        finally:
            log_client_activity_safe(current_thread_name, f"Tugas untuk SKU {sku} selesai")
    # Hapus 'pass' ini setelah Anda mengisi kode
    # =================================================
